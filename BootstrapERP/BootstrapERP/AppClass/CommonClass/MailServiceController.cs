...
namespace BootstrapERP.AppClass.CommonClass
{
    public class MailServiceController : DataAccessBase
    {
        public void eMailSendService(int companyID, MailServiceSetup objMailServiceSetup)
        {
            try
            {
                ...
                string sql = "select MailFrom,Passkey,SmtpServer,MailSubject,MailBody,IspAsswordWillSend,IsUserNameWillSend"
                            ...
                            + " where CompanyID= " + companyID + " and a.MailtypeID='" + objMailServiceSetup.MailtypeID + "'";
        ...
        public void eMailSendServiceHTML(int companyID, MailServiceSetup objMailServiceSetup)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                mailMessage.Subject = objMailServiceSetup.MailSubject.ToString();
                mailMessage.Body = objMailServiceSetup.MailBody.ToString();
                mailMessage.IsBodyHtml = true; //Edited
        ...
        public void eMailHTMLToCompany(int companyID, MailServiceSetup objMailServiceSetup)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (objMailServiceSetup.EmailTo == null)
                    {
                        objMailServiceSetup.EmailTo = dr["MailFrom"].ToString();
                    }
                    if (objMailServiceSetup.MailSubject == null)
                    {
                        objMailServiceSetup.MailSubject = dr["MailSubject"].ToString();
